    ...
    tabBox( width = 12,
      ...
      fluidRow(column(width=12,
       ...
       p(paste("The starting point is a ZIP file that contains one of three",
               ...
               "(one FASTQ for each sample); iii) a set of directories that contain",
               "(Applied Biosytems) AB1 files (each directory represents a sample and",
               "contains 1 or more AB1 files).  users will need to know the genomic",
               "location of the target sequence (\"guide\").  Once the set of reads",
               "are loaded, the user will be given an opportunity to specify labels",
               "for each sample (by default, these are taken from the file or",
               "directory names) and a grouping variable for colouring the label. ",
               "In the allele variant plot, variant sequences will be renumbered",
               "using the target.loc as the zero point.  Note that for a 20 nucleotide",
