data_setting <- bsButton("data_setting", 'Setting', type='action', icon =  icon("list-alt"), style = "primary", block = TRUE)
...
plot.text.size <- sliderInput("plot.text.size", "Plot text size : ", min = 0, max = 10, value = 2, step= 2)
...
x.angle <- sliderInput("x.angle", "X angle :", min = 1, max = 90, value = 90, step = 1, animate=TRUE)

axis.text.size  <- sliderInput("axis.text.size", "Text size :", min = 1, max = 20, value = 10)

ins.size <- sliderInput("ins.size", "size of the symbols within the plot area :", min = 1, max = 10, value = 3)
