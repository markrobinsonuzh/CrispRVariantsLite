...
mapFastQ <- reactive({
  ...
  ind <- paste0(GENOME_DIRECTORY, input$select_genome)
    ...
    if(file.exists(ind))
    {
      ...
      progress$set(message = "Creating  BAM files Mapping", value = 0)
      
      n <- length(v$fq_fnames)
      
      #Map, sort and index the bam files, remove the unsorted bams
      for(i in 1:n){
        progress$inc(1/n, detail = paste0("file of ", i, "/", n))
        cmd <- paste0("bwa mem ", bwa_index, " ", v$fq_fnames[i]," | samtools view -Sb - > ", v$bm_fnames[i])
        message(cmd, "\n"); system(cmd)
        indexBam(sortBam(v$bm_fnames[i],v$srt_bm_names[i]))
        unlink(v$bm_fnames[i])
      }
        createHTable()
    }
    else
